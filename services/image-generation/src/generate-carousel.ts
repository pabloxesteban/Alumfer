...
const SLIDES: SlideImage[] = [
  ...
  },
  {
    ...
    idea: {
      objective:
        'una ventana de aluminio que da a una avenida urbana con autos y un colectivo, vista de la calle a través del vidrio',
      setting: 'departamento sobre una avenida del Gran Buenos Aires',
      mood: 'día urbano, movimiento en la calle, contraste adentro/afuera',
      extraDetails: ['tráfico visible a través del vidrio', 'vereda y árboles'],
    },
  },
  {
    id: 'slide-5-humedad',
  ...
  },
  {
    ...
    idea: {
      objective:
        'un operario retirando una ventana vieja de una pared durante una renovación, ventana siendo desmontada',
      setting: 'obra de renovación en una casa',
      mood: 'escena de trabajo real, polvo, herramientas',
      extraDetails: ['pared abierta', 'ventana vieja fuera de lugar'],
    },
  },
  {
    id: 'slide-7-taller',
    label: 'El giro (taller/oficio)',
    idea: {
      objective:
        'taller de fabricación de aberturas de aluminio, manos de un operario cortando un perfil de aluminio en un corte a inglete de 45 grados',
      setting: 'taller metalúrgico con herramientas y máquinas',
      mood: 'luz cálida de taller, foco en el oficio y la precisión',
      extraDetails: ['perfil de aluminio en la sierra', 'chispas y virutas metálicas', 'ángulo de 45°'],
    },
  },
  {
    id: 'slide-8-cta',
...
async function main(): Promise<void> {
  ...
  console.log(`\n🎬 Generando ${SLIDES.length} imágenes del carrusel...\n`);
  const done: string[] = [];
  const failed: string[] = [];
  let totalCost = 0;
  ...
  for (const slide of SLIDES) {
    try {
      ...
      const result = await generateMarketingImage(
        { ...slide.idea, format: 'instagram-feed', quality: 'high' },
        { outputDir: outDir },
      );
